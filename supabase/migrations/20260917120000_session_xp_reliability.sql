...
REVOKE ALL ON FUNCTION public.session_actor_can_manage(UUID, UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.save_session_preclose(UUID, UUID, JSONB, INTEGER, JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.close_session_with_xp(UUID, UUID, JSONB, INTEGER, JSONB, TEXT, TEXT) FROM PUBLIC;
