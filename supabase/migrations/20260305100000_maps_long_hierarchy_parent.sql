-- Gerarchia mappe per campagne long: Mondo (unico) -> Continenti -> Regioni -> Città
ALTER TABLE public.maps
  ADD COLUMN IF NOT EXISTS parent_map_id UUID REFERENCES public.maps(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS maps_parent_map_id_idx ON public.maps(parent_map_id);

COMMENT ON COLUMN public.maps.parent_map_id IS 'Per campagne long: collegamento alla mappa di scala superiore (mondo->continente->regione->città).';

-- Una sola mappa "world" per campagna
CREATE UNIQUE INDEX IF NOT EXISTS maps_one_world_per_campaign_idx
  ON public.maps (campaign_id)
  WHERE map_type = 'world';
...
BEGIN
  ...
  IF NEW.map_type IN ('continent', 'region', 'city') AND NEW.parent_map_id IS NULL THEN
  ...
  END IF;
  IF NEW.map_type = 'region' AND p_type <> 'continent' THEN
    RAISE EXCEPTION 'Una regione deve essere collegata a un continente.';
  END IF;
  IF NEW.map_type = 'city' AND p_type <> 'region' THEN
    RAISE EXCEPTION 'Una città deve essere collegata a una regione.';
  END IF;
...
DROP TRIGGER IF EXISTS maps_long_parent_hierarchy_trg ON public.maps;
CREATE TRIGGER maps_long_parent_hierarchy_trg
  BEFORE INSERT OR UPDATE OF parent_map_id, map_type, campaign_id
  ON public.maps
  FOR EACH ROW
  EXECUTE FUNCTION public.maps_validate_long_parent_hierarchy();
