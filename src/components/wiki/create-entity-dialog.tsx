...
function appendCombatStatsToMarkdown(
  ...
  const parts = [hp ? `**PF:** ${hp}` : "", ac ? `**CA:** ${ac}` : ""].filter(
    Boolean,
  );
    ...
    <div className="flex min-w-0 flex-1 items-center gap-2">
      {icon ? (
        <span
          className={cn(
            "shrink-0",
            isPrivate ? "text-violet-300" : "text-barber-gold",
          )}
        >
          {icon}
        </span>
      ) : null}
      <h3 className="font-serif text-base font-semibold text-barber-paper sm:text-lg">
        {title}
      </h3>
  ...
  return (
    <section className={cn(wrapperBase, "p-4")}>
      <header className="mb-4 flex items-baseline justify-between gap-2">
        {headerInner}
        {hint ? (
          <span className="hidden shrink-0 text-xs text-barber-paper/55 sm:inline">
            {hint}
          </span>
        ) : null}
      </header>
...
export function CreateEntityDialog({
  ...
  const showCoreCheckbox =
    campaignType === "long" && (type === "npc" || type === "monster");
  ...
  const [magicEntityType, setMagicEntityType] =
    useState<WikiGeneratorEntityType>("npc");
  ...
  const [assistChatDraft, setAssistChatDraft] =
    useState<WikiMarkdownChatDraft | null>(null);
  ...
  const [wikiImageUrlPreset, setWikiImageUrlPreset] = useState<string | null>(
    null,
  );
  const [magicPortraitPreview, setMagicPortraitPreview] = useState<
    string | null
  >(null);
  type MagicDraft = {
    title: string;
  ...
  const aiProgressTimerRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  function parseStatsFromLoadedStatblock(source: string): {
    hp?: string;
    ac?: string;
    cr?: string;
  } {
    ...
    const crMatch =
      ...
      text.match(
        /\b(?:Sfida|GS|CR)\b\s*[:\-]?\s*([0-9]+(?:\/[0-9]+)?(?:\.[0-9]+)?)/i,
      );
    return {
      ac: acMatch?.[1]?.trim(),
      hp: hpMatch?.[1]?.trim(),
  ...
  function applyMonsterSheetStats(stats: {
    hp?: string;
    ac?: string;
    cr?: string;
  }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (type === "monster") {
      const currentStatblock =
        (typeof normalizedAttributes.statblock === "string"
          ? normalizedAttributes.statblock
          : ""
        ).trim() || monsterVerbatimStatblock.trim();
      if (currentStatblock) {
        ...
        const fallbackCr =
          parsed.cr ||
          (typeof existingCombat.cr === "string"
            ? existingCombat.cr.trim()
            : "") ||
          aiCr.trim();
        normalizedAttributes.combat_stats = {
          ...existingCombat,
          hp:
            parsed.hp ||
            (typeof existingCombat.hp === "string"
              ? existingCombat.hp.trim()
              : ""),
          ac:
            parsed.ac ||
            (typeof existingCombat.ac === "string"
              ? existingCombat.ac.trim()
              : ""),
          cr: fallbackCr || "",
        };
      }
    ...
    if (isAdmin && adminDraftsEnabled && adminOnly)
      formData.set("admin_only", "true");
    formData.set(
      "allowed_user_ids",
      JSON.stringify(visibility === "selective" ? selectedPlayerIds : []),
    );
    formData.set(
      "allowed_party_ids",
      JSON.stringify(visibility === "selective" ? selectedPartyIds : []),
    );
    if (sortOrder.trim() !== "") formData.set("sort_order", sortOrder.trim());
    if (showCoreCheckbox && isCore) formData.set("is_core", "on");
    if (showAiMemoryCheckbox && includeInAiMemory)
      formData.set("include_in_campaign_ai_memory", "on");
    ...
    if (
      !hasUploadedImage &&
      presetImageUrl &&
      isValidImageUrl(presetImageUrl)
    ) {
  ...
  async function injectGeneratedImageAsFile(imageUrl: string) {
    ...
    const targetInput = formEl.querySelector<HTMLInputElement>(
  ...
  function applyAssistChatDraft() {
    ...
    toast.success(
      "Bozza chat applicata al form. Controlla e salva quando pronto.",
    );
  }
  ...
  function buildAssistMarkdownExtraParams() {
    if (type === "monster") {
      return {
        cr: aiCr.trim(),
        verbatimMonsterStatblock: monsterVerbatimStatblock.trim(),
      };
    }
    if (type === "item") {
  ...
  async function handleMagicImageRefined(url: string) {
    ...
    setMagicDraft((prev) =>
      prev ? { ...prev, imageUrl: url, imageWarning: undefined } : prev,
    );
  }
  ...
  async function handleAssistGenerateImage() {
    ...
    if (!narrativeDescription) {
      toast.error(
        "Compila la descrizione narrativa nel campo contenuto prima di generare l'immagine.",
      );
      return;
    }
    ...
    try {
      ...
      success = true;
      toast.success(
        "Immagine AI generata e caricata nel file input originale.",
      );
    } catch {
  ...
  function handleOpenChange(next: boolean) {
    setOpen(next);
    if (next) {
      getWikiEntitiesForCampaign(campaignId).then(
        (r) => r.success && setWikiOptions(r.data),
      );
      getMapsForCampaign(campaignId).then(
        (r) => r.success && setMapOptions(r.data),
      );
    } else {
      setTitleValue("");
      setContentValue("");
  ...
  async function handleMagicGenerateImage() {
    ...
    try {
      ...
      setMagicDraft((prev) =>
        prev
          ? { ...prev, imageUrl: res.imageUrl, imageWarning: undefined }
          : prev,
      );
  ...
  function applyMagicDraftToForm() {
    ...
    const body = appendCombatStatsToMarkdown(
      magicDraft.content,
      magicDraft.hp,
      magicDraft.ac,
    );
    ...
    if (magicDraft.imageWarning) {
      toast.warning(`Immagine non generata: ${magicDraft.imageWarning}`, {
        duration: 8000,
      });
    }
    toast.success(
      "Bozza applicata al form. Controlla titolo, testo e immagine, poi premi Crea.",
    );
    setMagicOpen(false);
    setMagicDraft(null);
    setMagicPortraitPreview(null);
  ...
  const monsterStatblockLoaded =
    type === "monster" && monsterVerbatimStatblock.trim().length > 0;
        ...
        <DialogContent className="border-barber-gold/40 bg-barber-dark text-barber-paper sm:max-w-3xl">
          <DialogHeader className="space-y-1.5 text-left">
            ...
            <DialogDescription className="text-sm text-barber-paper/70">
              Aggiungi un NPC, un luogo, un mostro, un oggetto o una voce di
              lore. Se vuoi una bozza veloce, prova la{" "}
              <strong className="font-medium text-barber-gold">
                Bacchetta IA
              </strong>{" "}
              qui sotto.
            </DialogDescription>
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-3">
            ...
            >
              ...
              {campaignType === "long" && (
                <div className="space-y-2">
                  <Label htmlFor="create-linked-mission">
                    Missione collegata (opzionale)
                  </Label>
                  <select
                  ...
                  <p className="text-xs text-barber-paper/55">
                    Utile per ordinare il wiki e filtrare le immagini nella
                    Regia GM per missione.
                  </p>
                </div>
              )}
                  ...
                  {showAiMemoryCheckbox && (
                    ...
                    >
                      ...
                      <span className="min-w-0 space-y-0.5">
                        ...
                        <span className="block text-xs text-barber-paper/60">
                          Solo campagne lunghe: la voce entra nel contesto delle
                          generazioni IA future.
                        </span>
                      </span>
                    </label>
                  )}
            ...
            >
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-barber-paper/55">
                  Puoi compilare a mano oppure aprire l&apos;
                  <strong className="font-medium text-barber-gold">
                    Assistente IA
                  </strong>{" "}
                  qui sotto per una bozza guidata.
                </p>
              </div>
              ...
              {type === "npc" && (
                <>
                  <div className="grid gap-4 sm:grid-cols-3">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        {getAttr("class") &&
                          !WIKI_NPC_CLASS_OPTIONS.includes(
                            getAttr("class"),
                          ) && (
                            <option value={getAttr("class")}>
                              {getAttr("class")}
                            </option>
                          )}
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="attr-relationships">
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          setAttr("relationships", e.target.value)
                        }
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-barber-paper/55">
                      ...
                      per cercare lo statblock direttamente nei manuali.
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setAttr("combat_stats.hp", e.target.value)
                        }
                        placeholder="Es. 45"
                        className="bg-barber-dark border-barber-gold/30 text-barber-paper"
                        disabled={isLoading}
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setAttr("combat_stats.ac", e.target.value)
                        }
                        placeholder="Es. 15"
                        className="bg-barber-dark border-barber-gold/30 text-barber-paper"
                        disabled={isLoading}
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const xp = CHALLENGE_RATING_OPTIONS.find(
                            (o) => o.value === val,
                          )?.xp;
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="attr-attacks">
                        Attacchi / Azioni speciali
                      </Label>
                      <Textarea
                        id="attr-attacks"
                        value={getAttr("combat_stats.attacks")}
                        onChange={(e) =>
                          setAttr("combat_stats.attacks", e.target.value)
                        }
            ...
            >
              <p className="text-xs leading-relaxed text-barber-paper/65">
                ...
                <strong className="font-medium text-barber-paper">
                  Storia / Descrizione
                </strong>{" "}
                e (se applicabile) statblock e statistiche. Puoi sempre rivedere
                il risultato a mano prima di salvare.
              </p>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {monsterStatblockLoaded ? (
                      ...
                      <StatusBadge tone="warning">
                        Richiesto per generare
                      </StatusBadge>
                    )}
                  </div>
                  <p className="text-xs leading-snug text-barber-paper/65">
                    Scegli un mostro dalla lista oppure cerca per nome. Lo
                    statblock arriva direttamente dai PDF importati: l&apos;IA
                    genera solo la parte narrativa.
                  </p>
                  ...
                  <div className="space-y-2 rounded-md border border-barber-gold/20 bg-barber-dark/45 p-2.5">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                      <select
                        ...
                        onChange={(e) =>
                          setSelectedBestiaryListId(e.target.value)
                        }
                        disabled={
                          isLoading || assistChatLoading || bestiaryListLoading
                        }
                        ...
                        {bestiaryHits.map((h) => (
                          ...
                          >
                            ...
                            <Button
                              ...
                              variant="outline"
                              disabled={
                                loadingChunkId === h.id || assistChatLoading
                              }
                ...
                <div className="space-y-3 rounded-lg border border-barber-gold/30 bg-barber-dark/65 p-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="space-y-1">
                      <Label className="text-xs text-barber-paper/80">
                        Razza
                      </Label>
                      <select
                        value={npcAiRace}
                  ...
                  <p className="text-[11px] text-barber-paper/55">
                    La scheda userà solo manuali non esclusi nei paletti
                    campagna.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-3 rounded-lg border border-barber-gold/30 bg-barber-dark/65 p-3">
                ...
                {!assistChatReady() ? (
                  <p className="text-xs text-amber-400/90">
                    Compila titolo
                    {type === "monster"
                      ? " e carica lo statblock dal bestiario"
                      : ""}
                    {type === "npc" ? ", razza, classe e livello" : ""} prima di
                    chattare con l&apos;IA.
                  </p>
                ) : null}
                ...
                {type === "monster" ? (
                  <div className="space-y-2">
                    <Label
                      htmlFor="assist-cr"
                      className="text-xs text-barber-paper/80"
                    >
              ...
              <div className="space-y-3 rounded-lg border border-barber-gold/30 bg-barber-dark/65 p-3">
                ...
                <p className="text-xs text-barber-paper/65">
                  Genera un&apos;immagine coerente con la{" "}
                  <strong className="font-medium text-barber-paper">
                    descrizione
                  </strong>{" "}
                  già scritta sopra. L&apos;immagine viene caricata
                  automaticamente nel campo «Immagine» qui sotto.
                  {type === "monster" || type === "npc" ? (
                    <span className="mt-1 block text-barber-gold/85">
                      ...
                      l&apos;inquadratura predefinita è{" "}
                      <strong className="font-medium text-barber-paper">
                        a figura intera in piedi
                      </strong>
                      , salvo che nel prompt chiedi esplicitamente una posa
                      seduta.
                    </span>
                  ) : type === "item" ? (
                    <span className="mt-1 block text-barber-gold/85">
                      ...
                      l&apos;immagine è centrata sull&apos;item isolato, senza
                      personaggi.
                    </span>
                  ) : type === "lore" ? (
                    <span className="mt-1 block text-barber-gold/85">
                      ...
                      non ci sono vincoli di soggetto: l&apos;immagine segue il
                      testo (oggetto, persona, mappa, scena…).
                    </span>
                  ) : null}
                </p>
                <Button
            ...
            >
              <p className="text-xs text-barber-paper/60">
                Collega questa voce ad altre voci wiki o a mappe. Le relazioni
                appariranno nella Mappa Concettuale (Solo GM).
              </p>
                  ...
                  <label className="flex items-center gap-2 rounded-md border border-violet-500/30 bg-violet-500/10 p-3 text-sm text-violet-100">
                    ...
                    <span>
                      ...
                      <span className="text-xs text-violet-200/70">
                        Crea come bozza segreta; potrai rilasciarla in seguito.
                      </span>
                    </span>
                  </label>
                ) : null}
      ...
      <Dialog open={magicOpen} onOpenChange={handleMagicDialogOpenChange}>
        <DialogContent className="border-barber-gold/40 bg-barber-dark text-barber-paper sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-barber-paper/70">
              Prompt iniziale → bozza testo → chiedi modifiche in chat. Quando
              il testo ti convince puoi generare l&apos;immagine (NPC, luoghi,
              oggetti e lore).
            </DialogDescription>
          </DialogHeader>
            ...
            {magicDraft ? (
              <div className="space-y-2">
                ...
                {magicDraft.imageWarning && !magicPortraitPreview ? (
                  <p className="text-xs text-amber-400/90">
                    Immagine: {magicDraft.imageWarning}
                  </p>
                ) : null}
                {magicPortraitPreview ? (
                  <>
