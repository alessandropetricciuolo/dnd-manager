...
export async function bulkImportMaps(
  campaignId: string,
  input: unknown,
  adminOnly = false,
  ...
  try {
    ...
    if (!accessResult.ok)
      return { success: false, message: "Autorizzazione non verificabile." };
    if (adminOnly) {
      try {
        await assertCanManageAdminContent(
          accessResult.access,
          campaignId,
          supabase as never,
        );
        ...
        return {
          success: false,
          message:
            "Solo un Admin può importare mappe Solo Admin in una campagna abilitata.",
        };
      }
    }
    const {
    ...
    if (authError || !allowed)
      return {
        success: false,
        message: "Non puoi gestire le mappe di questa campagna.",
      };
    ...
    if (campaignError || !campaign)
      throw new Error("Campagna non disponibile.");
    const { data: existing, error: readError } = await supabase
      .from("maps")
      .select("id, map_type")
      .eq("campaign_id", campaignId);
    if (readError)
      throw new Error("Impossibile verificare le mappe esistenti.");
    ...
    if (error)
      return {
        success: false,
        message: `Nessuna mappa importata: ${error.message}`,
      };
    if (adminOnly)
