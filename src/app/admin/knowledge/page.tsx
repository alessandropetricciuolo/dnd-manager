...
import { ingestTxtManualAction } from "@/lib/actions/ingest-manuals";
...
export default async function AdminKnowledgePage({ searchParams }: KnowledgePageProps) {
  ...
  async function runManualIngest() {
    ...
    const nextMessage = result.success
      ? `Ingestion completata: ${result.inserted}/${result.chunks} chunk inseriti.`
      : result.message;
      ...
      <div className="mx-auto w-full max-w-3xl space-y-6">
        <header className="space-y-2">
          ...
          <p className="text-sm text-barber-paper/70">
            Ingestion manuali `.txt` da `public/manuals/` con embeddings (`all-MiniLM-L6-v2`) in `manuals_knowledge`.
          </p>
        </header>
