...
export async function buildStructuredWikiTextSystemPrompt(
  ...
  if (error) {
    return { error: error.message ?? "Impossibile caricare il contesto campagna." };
  }
  ...
  const ruleBase = `Sei un Game Master di D&D 5e. Devi generare o aggiornare un ${ENTITY_LABEL_IT[entityType]}. Tutte le statistiche devono rispettare fedelmente le regole di D&D 5e.`;
  const contextBlock = buildCampaignContextBlock(ctx);
  const outputForce =
    "Rispondi ESCLUSIVAMENTE con un oggetto JSON valido contenente le chiavi: title (string), content (string, formattato in markdown), hp e ac (string o numero; usa null se non applicabili per questa entità, es. lore senza combattimento). Non usare markdown fuori dal valore della chiave content. In chat multi-turno, quando il Master chiede modifiche, rigenera l'intero JSON aggiornato.";

  const systemPrompt = [ruleBase, contextBlock, wikiMemory, outputForce]
...
/** Formato leggibile del draft per la UI chat. */
export function formatStructuredWikiDraftForChat(draft: WikiAiTextGeneration): string {
  const stats: string[] = [];
  if (draft.hp) stats.push(`PF: ${draft.hp}`);
  if (draft.ac) stats.push(`CA: ${draft.ac}`);
  const statsLine = stats.length ? `\n\n_${stats.join(" · ")}_` : "";
  return `**${draft.title}**${statsLine}\n\n${draft.content}`;
}
...
/** Parsa la risposta JSON del modello per la bacchetta IA / chat strutturata. */
export function parseStructuredWikiJson(rawText: string): TextResult {
  let parsed: unknown;
  try {
...
export async function generateContextualText(
  ...
  try {
    ...
    const parsedResult = parseStructuredWikiJson(rawText);
    if (!parsedResult.ok) {
      return parsedResult;
    }
