...
4. **AI (opzionale):** per le Server Action che usano Hugging Face Inference usa **`HUGGINGFACE_API_KEY`** oppure **`HF_TOKEN`** (stesso token; vedi [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens)). **Su Vercel:** *Settings → Environment Variables* → **Key** `HUGGINGFACE_API_KEY`, **Value** solo `hf_...` (non scrivere `HUGGINGFACE_API_KEY=` nel value). Spunta *Production* (e *Preview* se serve), **Salva**, **Redeploy**. Nei log delle Functions cerca `DEBUG API KEY (Hugging Face)` (lunghezze 0 = variabile assente nel deployment). Applica anche la migrazione `supabase/migrations/20260301130000_campaigns_ai_context.sql` (colonna `campaigns.ai_context`).
