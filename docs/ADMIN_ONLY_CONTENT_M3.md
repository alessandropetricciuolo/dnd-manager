...
|---|---|---|---|
| Solo Admin solo con flag attivo (release escluso) | `createEntity`, `updateEntity`, `uploadMap`, `updateMap` risolvono identità con `resolveAdminContentAccess` e usano `resolveAdminOnlyTransition` | suite M3 16/16; lint focalizzato | il flusso browser autenticato reale è demandato a M7 |
...
| Rilascio/ritiro espliciti | `updateEntity` e `updateMap` accettano intenti distinti; il release verifica Admin e stato corrente anche a flag spento; UI mostra release per righe protette anche a flag spento | test contratto + lint; audit post-scrittura | browser reale demandato a M7 |
